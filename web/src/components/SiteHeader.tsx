  ...
  return (
    <header className="sticky top-0 z-40 flex h-14 items-center gap-3 border-b border-[var(--color-line)] bg-[rgba(10,12,16,0.82)] px-5 backdrop-blur">
      <Link href="/" className="flex items-center gap-2 font-bold tracking-tight">
        <span>🦞</span>
        <span>OpenClaw <span className="text-[var(--color-fg-mut)]">on</span> <span className="text-[var(--color-rh-bright)]">OpenShift</span></span>
      </Link>
      <div className="ml-auto flex items-center gap-2 text-[0.72rem] text-[var(--color-fg-mut)]">
        ...
        <span className="hidden rounded-full border border-[var(--color-line-2)] px-2.5 py-1 sm:inline">WeAreDevelopers 2026</span>
      </div>
    </header>
  );
}
