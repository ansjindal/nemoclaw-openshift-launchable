      ...
      {CURRICULUM.map((part) => (
        <div key={part.id}>
          <div className="px-2 pb-2">
            <div className={`text-[0.82rem] font-bold ${part.accent === "rh" ? "text-[var(--color-rh-bright)]" : part.accent === "nv" ? "text-[var(--color-nv-bright)]" : "text-white"}`}>{part.title}</div>
          ...
          {part.lessons.map((l) => {
            ...
            return (
              <Link key={l.slug} href={`/learn/${l.slug}`}
                className={`flex items-center gap-2 rounded-md px-2 py-1.5 ${active ? "bg-[var(--color-panel-2)] text-white" : "text-[var(--color-fg-dim)] hover:bg-[var(--color-panel)] hover:text-white"}`}
